<template>
  <div class="rag-page">
    ...
    <div v-if="showKnowledgeBaseEditor" class="modal-backdrop" @click.self="closeKnowledgeBaseEditor">
      <form class="rag-modal" role="dialog" aria-modal="true" aria-labelledby="knowledge-base-dialog-title" @keydown.esc.stop.prevent="closeKnowledgeBaseEditor" @submit.prevent="saveKnowledgeBase">
        <header><div><span>{{ editingKnowledgeBaseId ? 'EDIT LIBRARY' : 'NEW LIBRARY' }}</span><h2 id="knowledge-base-dialog-title">{{ editingKnowledgeBaseId ? '编辑知识库' : '建立知识库' }}</h2></div><button type="button" aria-label="关闭" @click="closeKnowledgeBaseEditor"><X :size="19" /></button></header>
    ...
    <div v-if="showKnowledgeBaseDeleteDialog && selectedKnowledgeBase" class="modal-backdrop" @click.self="closeKnowledgeBaseDeletion">
      <form class="rag-modal rag-modal--danger" role="dialog" aria-modal="true" aria-labelledby="knowledge-base-delete-title" @keydown.esc.stop.prevent="closeKnowledgeBaseDeletion" @submit.prevent="requestKnowledgeBaseDelete">
        <header><div><span>IRREVERSIBLE DELETE</span><h2 id="knowledge-base-delete-title">删除整个知识库</h2></div><button type="button" aria-label="关闭" :disabled="knowledgeBaseDeleteBusy" @click="closeKnowledgeBaseDeletion"><X :size="19" /></button></header>
    ...
    <div v-if="showProfileEditor" class="modal-backdrop" @click.self="showProfileEditor = false">
      <form class="rag-modal rag-modal--wide" role="dialog" aria-modal="true" aria-labelledby="rag-profile-dialog-title" @keydown.esc.stop.prevent="showProfileEditor = false" @submit.prevent="saveProfile">
        <header><div><span>RETRIEVAL PROFILE</span><h2 id="rag-profile-dialog-title">{{ editingProfileId ? '编辑检索策略' : '新建检索策略' }}</h2></div><button type="button" aria-label="关闭" @click="showProfileEditor = false"><X :size="19" /></button></header>
...
<style scoped>
.rag-page { min-width: 0; min-height: calc(100vh - 72px); padding: 30px; background: var(--bg); }
...
.rag-hero h1 { margin: 6px 0 8px; font-family: "Newsreader Variable", "Songti SC", serif; font-size: clamp(34px, 4vw, 58px); line-height: 1; letter-spacing: -.055em; }
...
.library-rail { display: flex; flex-direction: column; border-right: 1px solid var(--line); background: #f0f0e9; }
...
.rag-button { min-height: 32px; padding: 0 10px; border-radius: 5px; font-size: 11px; }
.rag-button--danger { min-height: 32px; }
.rag-button--primary { box-shadow: none; }
.rag-workbench { border-radius: 10px; box-shadow: var(--shadow-sm); }
.library-rail, .document-stage, .retrieval-lab { padding: 18px; }
.library-card, .document-row, .task-card, .kb-delete-progress, .danger-zone { border-radius: 7px; }
.library-glyph { border-radius: 5px; }
.count-chip { border-radius: 4px; }
.rag-modal { border-radius: 10px; }
.rag-modal h2 { font-family: "Newsreader Variable", "Songti SC", serif; }
.skeleton-list,.document-skeleton { display: grid; gap: 9px; }.skeleton-list span,.document-skeleton span { border-radius: 12px; background: var(--surface-muted); animation: shimmer 1.4s infinite alternate; }.skeleton-list span { height: 68px; }.document-skeleton span { height: 76px; }
.spin { animation: spin .9s linear infinite; }@keyframes spin { to { transform: rotate(360deg); } }@keyframes shimmer { to { opacity: .48; } }
...
@media (max-width: 800px) { .rag-button { min-height: 40px; }.rag-workbench { border-radius: 9px; } }
