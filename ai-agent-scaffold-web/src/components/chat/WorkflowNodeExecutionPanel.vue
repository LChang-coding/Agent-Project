<template>
  <details class="execution-panel" :open="run.status === 'running'">
    ...
    <div class="execution-body">
      <ol v-if="executionTimeline.length" class="execution-timeline" aria-label="Agent 运行时间线">
        <li v-for="item in executionTimeline" :key="`${item.kind}-${item.id}`">
          <div v-if="item.kind === 'activity'" :class="['activity-item', `activity--${item.activity.status}`]">
            <span class="activity-mark" aria-hidden="true" />
            <div><strong>{{ item.activity.label }}</strong><small v-if="item.activity.detail">{{ item.activity.detail }}</small></div>
            <em>{{ activityStatus(item.activity.status) }}<template v-if="item.activity.costMs !== undefined"> · {{ item.activity.costMs }}ms</template></em>
          </div>
          <div v-else class="react-turn">
          <details v-if="item.turn.thinking" :class="['thinking-block', { 'thinking-block--active': isThinkingActive(item) }]" :open="isThinkingActive(item)">
            <summary>
              <span :class="['thinking-wave', { 'thinking-wave--active': isThinkingActive(item) }]" aria-hidden="true"><i /><i /><i /></span>
              <strong>{{ isThinkingActive(item) ? '正在思考' : `思考 ${item.reactIndex + 1}` }}</strong>
              <small>{{ item.turn.thinking.length }} 字</small>
            </summary>
            <pre>{{ item.turn.thinking }}</pre>
          </details>
          <div v-for="call in item.turn.tools" :key="call.functionCallId" :class="['react-tool', `react-tool--${call.status}`]">
            <span class="activity-mark" aria-hidden="true" />
...
import type { AgentReactTurnView, AgentRunActivityView, WorkflowRunViewState } from '@/types/intelligent-workflow';
...
type TimelineItem =
  | { kind: 'activity'; id: string; sequence: number; activity: AgentRunActivityView }
  | { kind: 'react'; id: string; sequence: number; turn: AgentReactTurnView; reactIndex: number };
const executionTimeline = computed<TimelineItem[]>(() => [
  ...props.run.activities.map((activity) => ({ kind: 'activity' as const, id: activity.id, sequence: activity.sequence, activity })),
  ...props.run.reactTurns.map((turn, reactIndex) => ({ kind: 'react' as const, id: turn.id, sequence: turn.sequence, turn, reactIndex })),
...
function isThinkingActive(item: TimelineItem) {
  const last = executionTimeline.value.at(-1);
  return item.kind === 'react' && props.run.status === 'running' && !props.run.waitingAll
    && last?.kind === 'react' && last.id === item.id && item.turn.tools.length === 0;
}
...
.thinking-wave{display:flex;align-items:flex-end;width:17px;height:13px;gap:2px}.thinking-wave i{width:3px;height:5px;border-radius:2px;background:var(--accent-deep);opacity:.66}.thinking-wave i:nth-child(2){height:10px}.thinking-wave i:nth-child(3){height:7px}.thinking-wave--active i{animation:wave 1s ease-in-out infinite}.thinking-wave--active i:nth-child(2){animation-delay:.12s}.thinking-wave--active i:nth-child(3){animation-delay:.24s}
...
.node-list{display:grid;margin:0;padding:0;gap:6px;list-style:none}.activity-item{display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:start;padding:9px 10px;gap:9px;border-radius:8px;background:rgb(255 255 255/.54)}.activity-mark{width:9px;height:9px;margin-top:3px;border:2px solid var(--line-strong);border-radius:50%}.activity--running .activity-mark{border-color:var(--accent-deep);border-top-color:transparent;animation:spin .8s linear infinite}.activity--completed .activity-mark{border-color:var(--success);background:var(--success)}.activity--failed .activity-mark{border-color:var(--danger);background:var(--danger)}.activity--waiting .activity-mark{border-color:var(--warning);background:var(--warning)}.activity-item strong,.activity-item small{display:block}.activity-item small{margin-top:3px;color:var(--muted)}.activity-item em{color:var(--muted);font-size:11px;font-style:normal}
