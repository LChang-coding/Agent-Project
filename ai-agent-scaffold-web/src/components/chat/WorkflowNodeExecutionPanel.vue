<template>
  <details class="execution-panel" :open="run.status === 'running'">
    ...
    <div class="execution-body">
      <details v-if="run.thinking" class="thinking-block" :open="run.status === 'running'">
        <summary>
          <span class="thinking-wave" aria-hidden="true"><i /><i /><i /></span>
          <strong>{{ run.status === 'running' ? '正在思考' : '思考过程' }}</strong>
          <small>{{ run.thinking.length }} 字</small>
        </summary>
...
const eventSummary = computed(() => `${props.run.nodes.length + props.run.activities.length} 个步骤 · #${props.run.lastSequence}`);
