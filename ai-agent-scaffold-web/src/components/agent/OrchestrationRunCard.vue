<template>
  <section v-if="snapshot && currentRun" class="run-card" aria-live="polite">
    <header>
      <div>
    ...
    <div class="task-grid">
      <button v-for="task in currentTasks" :key="task.taskId" type="button"
...
const props = defineProps<{ snapshot?: SessionOrchestrationSnapshot; agents: AiAgentConfig[]; stopping?: boolean }>();
defineEmits<{ 'select-task': [taskId: string]; stop: [] }>();
