...
### 2026-07-15：XXL-JOB 管理端公网访问执行计划

#### 需求解释

- “定时器没启动”指 XXL-JOB 中 `scheduleReconcileJobHandler` 与 `scheduleDispatchJobHandler` 两条业务唤醒任务的 `trigger_status=0`，不是 Admin/MySQL 容器停止；此前停用是因为服务器没有常驻业务执行器，避免空调度持续失败。
- 两条任务的初始 Cron 与默认停用状态由 `docs/dev-ops/xxl-job/bootstrap-business-jobs.sql` 配置；运行时可在 XXL-JOB 管理台“任务管理”中启停，前提是 `ai-agent-scheduler` 执行器已有在线地址。

#### 本次执行步骤

1. 只读检查服务器 Compose、`.env` 监听配置、容器健康、宿主机防火墙与 8080 当前占用，确认变更前状态和回滚值；
2. 备份服务器 `.env`，将 Admin 监听从 `127.0.0.1` 改为 `0.0.0.0`，同时把仓库 `.env.example` 与 README 的直接公网访问说明对齐；
3. 仅重建 XXL-JOB Admin 容器，不改 MySQL 数据卷、不启用两条业务任务；若宿主机防火墙启用，则只增加 TCP/8080 规则；
4. 从服务器本机和本地分别验证登录页、公网 IP 访问、容器健康、端口监听及两条业务任务仍为停用；失败则恢复 `.env` 备份并重建 Admin；
5. 更新本机 `codex.md` 的 XXL-JOB 地址，追加实际变更与验证结果；精确暂存任务相关文档，使用中文提交，不纳入凭据和用户既有文件。

#### 风险边界

- 本次按用户要求提供 `http://公网IP:8080` 直连，管理登录将暴露在公网且没有 TLS；依赖高强度密码作为当前保护，不将其描述为长期最佳实践。
- 本次只改变管理端网络入口，不擅自启动业务定时任务；启用任务前仍需让业务应用/执行器常驻并能被 Admin 回调。

#### 实际执行结果

- 变更前确认 Admin/MySQL 已连续运行约 17/18 小时，Admin 监听 `127.0.0.1:8080`，MySQL 仅在 Compose 内网；服务器未启用 firewalld，两条业务任务的 `trigger_status` 均为 0；
- 服务器 `.env` 已生成权限为 600 的时间戳备份，并将 `XXL_JOB_ADMIN_BIND_IP` 改为 `0.0.0.0`；仓库 `.env.example` 同步公网监听默认值，README 补充直连地址、定时任务配置位置、启用前提及明文 HTTP 风险；
- 第一次 Compose 热重建因旧容器占用 8080 未完成切换，自动恢复原 `.env` 与旧容器；随后只停止/移除 Admin 容器并重建成功，MySQL 容器、数据库和数据卷全程未停止或重建；
- 最终宿主机监听为 `0.0.0.0:8080`，本地访问 `http://69.165.65.123:8080/xxl-job-admin/` 返回 HTTP 302 并正确跳转公网登录页，服务器 loopback 同样返回 302；
- 使用现有 Admin 凭据登录返回业务码 200/Success；Admin 重启次数为 0，XXL MySQL 为 healthy，Compose 配置检查通过；
- 两条任务仍分别保持 `0 */5 * * * ?`（配置对账）与 `*/5 * * * * ?`（到期派发），且 `trigger_status=0`，公网发布没有意外启动业务调度；
- 本机 `codex.md` 已把 XXL-JOB 地址更新为 `http://69.165.65.123:8080/xxl-job-admin` 并标注当前未配置 TLS；该文件继续本机排除，不提交凭据。

### 2026-07-15：XXL-JOB Admin 登录凭据纠正计划

#### 问题与执行步骤

1. 对比本机 `codex.md` 记录值、服务器 `.env` 当前明文值、数据库 `xxl_job_user` 中的摘要，以及部署脚本使用的 SHA-256 计算方式，不输出任何真实密码或会话令牌；
2. 使用与浏览器相同的公网 `/auth/doLogin` 表单请求验证记录凭据，明确是文档失真、数据库摘要失配、输入格式还是服务端登录问题；
3. 若数据库与 `.env` 不一致，以服务器 `.env` 为唯一部署来源重新写入摘要；若文档不一致则修正本机 `codex.md`；必要时改进部署脚本，避免 shell 解析导致密码值失真；
4. 再次验证公网登录返回成功、认证后任务页可访问、Admin/MySQL 健康且业务任务未被意外启用；
5. 将原因、实际改动与验证结果追加到本节，精确暂存不含凭据的文档/脚本并使用中文提交，不纳入 `codex.md`、运行日志和用户既有未跟踪目录。

#### 实际执行结果

- 排查时确认原 `codex.md` 明文与服务器 `.env` 完全相同，其 SHA-256 与 `xxl_job_user.admin` 数据库摘要一致；使用原值从本机请求公网 `/auth/doLogin` 返回业务码 200，认证后任务页可正常识别，原凭据在服务端实际上有效；
- 为消除复制反引号、前后空格或浏览器旧凭据造成的继续混淆，已主动轮换为新的 24 位纯字母数字密码；轮换前在服务器生成权限 600 的 `.env` 时间戳备份；
- 服务器 `.env` 与数据库用户摘要已原子对齐，本机 `codex.md` 同步新值；真实密码仍未进入仓库、日志、计划文档或提交信息；
- 使用新值从公网登录返回业务码 200，认证后“任务管理”页面可访问；再次使用旧值已被拒绝，证明轮换即时生效；
- 最终 `codex.md` 与服务器 `.env` 相同，数据库摘要匹配；Admin 状态 running、重启次数 0，XXL MySQL healthy，启用中的业务任务数仍为 0；
- 部署脚本已使用无换行 SHA-256 并通过 `--data-urlencode` 等价验证，本次未发现摘要算法或服务端登录缺陷，因此没有为了现象扩大修改部署代码。

### 2026-07-15：XXL-JOB Admin 真实浏览器登录复验计划

#### 执行步骤

1. 采用 `browser-qa` 从公网登录页开始，检查实际表单字段、页面错误提示、网络请求和登录后的页面跳转，不再只以 Curl 接口成功作为结论；
2. 将当前包含 `0/O、I/i/1` 的随机密码轮换为长度足够、无歧义且便于直接复制的纯字母数字密码，轮换前备份服务器 `.env`；
3. 同步更新服务器 `.env`、`xxl_job_user` 摘要和本机 `codex.md`，失败时恢复备份与旧摘要；
4. 依次完成真实浏览器登录、公网接口登录、认证后任务页、旧密码失效、数据库摘要、容器健康和任务启用状态验证；
5. 将实际结果追加到本节，提交不含密码的计划记录，继续排除本机凭据文件和用户既有文件。

#### 验收标准

- 在可视浏览器中手工填写 `admin` 和新密码后，页面明确进入任务管理界面，不出现“账号或密码错误”；
- 新密码公网接口成功、旧密码失败，`codex.md`/服务器 `.env`/数据库摘要一致；
- Admin/MySQL 健康且两条业务任务仍保持停用。

#### 实际执行结果

- 真实 Chrome 首次复现“账号或密码错误”：页面密码输入框定义了 `maxlength=20`，自动填写 24 位密码后浏览器只保留前 20 位；此前 Curl 绕过 HTML 长度限制并提交完整密码，所以接口成功结论不能代表网页登录成功；
- 根因不是账号、数据库摘要算法或公网地址，而是部署允许生成超过 XXL-JOB 3.4.0 登录页上限的 Admin 密码，原 64 位值和第一次轮换的 24 位值都无法通过标准网页完整提交；
- Admin 密码已再次轮换为恰好 20 位、便于复制的字母数字值；服务器 `.env`、数据库摘要和本机 `codex.md` 已同步，服务器保留权限 600 的轮换前备份；
- `deploy.sh` 新增 Admin 密码 4~20 字符 fail-fast 校验，`.env.example` 与 README 同步说明网页登录长度限制，防止后续重新部署复发；
- 使用本机 Google Chrome 真实填写账号密码并点击登录后，`/auth/doLogin` 返回业务码 200，页面进入 `#/xxl-job-admin/dashboard`，可见任务管理，未出现登录错误或控制台错误；
- 公网 Curl 补充验证新密码成功、上一组密码失败；`codex.md`/服务器 `.env`/数据库摘要一致，Admin running 且重启次数 0、MySQL healthy、启用任务数 0；
- `bash -n deploy.sh`、超长密码判定分支和任务文件 `git diff --check` 均通过；本闭环修正了此前只验证接口、没有验证浏览器输入约束的测试缺口。

### 2026-07-15：服务器常驻业务执行器部署计划

#### 授权与架构边界

- 用户明确要求将业务执行器部署到服务器；这会对 `codex.md` 既有“服务器只部署中间件、本地运行项目代码”形成一次明确的生产部署例外，本闭环仅部署当前 Spring Boot 模块化单体作为常驻执行器，不擅自迁移前端或其他本地开发流程。
- 执行器与现有 XXL-JOB Admin 使用相同 access token，敏感配置只保存在服务器权限受限环境文件中；构建产物不得包含本机 `secrets.properties`、JWT、模型或中间件明文凭据。

#### 执行步骤

1. 只读审计应用启动入口、Maven 制品、运行配置覆盖方式、健康检查、XXL-JOB 注册参数和服务器 Java/内存/端口/systemd/网络现状，确定最小部署拓扑与回滚路径；
2. 在本地执行 Java 17 相关测试与干净构建，扫描最终 JAR 不包含本机密钥文件，计算 SHA-256；
3. 新增可提交的 systemd/环境变量模板与部署说明，服务器创建独立运行用户、版本目录、权限 600 的真实环境文件、current 软链和日志目录；
4. 上传版本化 JAR，先以前台/临时服务方式验证应用、数据库、中间件和 XXL-JOB 自动注册；失败时停止服务并恢复前一版本/current；
5. 安装并启用 systemd 常驻服务，验证进程健康、端口边界、开机自启、Admin 执行器地址非空和两个 Handler 手动调用成功；
6. 先启用配置对账任务并观察，再启用每 5 秒派发任务；验证至少一个 Cron 周期无地址路由失败、无重复执行或异常费用；
7. 执行服务重启恢复测试，核对自动注销/重新注册、Admin/MySQL/应用日志与任务状态；回填实际结果并用中文本地提交。

#### 验收与回滚

- Admin “执行器管理”中 `ai-agent-scheduler` 至少存在一个在线地址，`scheduleReconcileJobHandler` 与 `scheduleDispatchJobHandler` 手动/周期触发均成功；
- systemd 为 active/enabled，应用健康检查成功，服务器重启服务后执行器自动恢复注册；
- 对公网不额外开放业务 API 或 9999，Admin 必须能通过受限服务器网络访问执行器；
- 任一步失败时立即停用两条任务、停止新服务、恢复 current 到上一版本或删除本次首装服务，不修改业务数据迁移结果和 XXL-JOB MySQL 数据卷。

#### 用户中止与清理结果

- 用户在镜像构建期间明确补充“不要将本地的这个项目上传服务器”，该要求覆盖本节原部署授权；已立即中断构建，不再启动应用、不启用任务，也不继续常驻执行器方案；
- 中止前已完成本地 5 项调度测试、Java 17 七模块打包和 JAR 无密钥检查，并已将 JAR/外部密钥副本上传到临时部署目录；镜像尚未构建完成，Compose 尚未启动；
- 已删除服务器 `/opt/ai-agent-executor` 整个临时部署目录，包括 JAR、Dockerfile、Compose、`.env` 和密钥副本；确认目录、容器、镜像标签和数据卷均不存在；
- 已撤销本地尚未提交的 Maven 密钥排除、可选配置导入及 `docs/dev-ops/agent-executor` 部署资产，项目代码保持部署前状态；
- 最终 XXL-JOB Admin running 且重启次数 0、XXL MySQL healthy、启用任务数 0、`ai-agent-scheduler` 在线注册数 0，原中间件状态未受影响；
- 后续若要解决 XXL-JOB 执行器地址为空，必须采用“不上传本地项目”的独立方案，例如只在本地常驻运行并建立受控反向隧道，或另行开发/构建不包含本地项目源码与制品的远程执行器；本次不替用户选择。
