...
/**
 * 不依赖任何词表和语料统计的稀疏向量编码器，用于关键词召回这一路。
 *
 * <p>解决什么问题：混合检索里除了语义相近的稠密向量，还需要一路「字面命中」的稀疏向量，
 * 否则专有名词、型号、人名一旦语义模型不认识就召不回来。传统 BM25 需要维护全局词表和文档频率，
 * 在多租户场景下要么按租户各建一份词表（成本高、还会随数据变化漂移），要么共用一份（跨租户泄露统计信息）。
 * 本类改用「哈希技巧」：把词项直接哈希到固定维度的桶里，不存词表、不统计语料，
 * 因此同一段文字在任何时间、任何机器上都会编出完全一样的向量，索引和查询永远对得上。</p>
 *
 * <p>属于哪一层：领域层（domain）对稀疏编码端口 SparseEncoderPort 的默认实现。无状态、线程安全。</p>
 *
 * <p>谁会调用它：摄取 Worker 在给分块建索引时批量调用（走 Spring 装配的端口），
 * 检索链路在把用户问题转成稀疏向量时也走同一实现。两边必须是同一套算法，否则查询和索引根本不在同一个空间里。</p>
 *
 * <p>它向下调用什么：只用 JDK 的 Unicode 归一化与文字脚本判定，不调任何外部服务、不读库。</p>
 *
 * <p>它不负责什么：不做同义词扩展、不做停用词过滤、不做真正的 IDF 加权（没有语料统计就没有 IDF）、
 * 不做稠密语义向量、不负责向量落库和检索。</p>
 */
