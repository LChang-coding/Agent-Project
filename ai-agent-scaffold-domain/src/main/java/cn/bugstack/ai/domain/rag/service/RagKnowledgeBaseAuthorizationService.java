...
/**
 * RAG 知识库这一侧的「你是谁、你能不能动它」统一守门人。
 *
 * <p>解决什么问题：知识库里放的是租户的私有资料，一旦越权就等于把别家公司的文档喂给了别人的模型。
 * 所有增删改查前都必须先确认三件事：身份是不是可信的、角色够不够管理员、这个资源是不是当前租户的。
 * 把这三件事收拢到一个类里，避免每个管理服务各写一套判断、漏掉一处就形成越权口子。</p>
 *
 * <p>属于哪一层：领域层（domain）的授权规则服务。只做判断、只抛异常，不查库、不返回数据。</p>
 *
 * <p>谁会调用它：知识库管理、文档管理与上传、文档删除、知识库删除、检索配置维护、检索调试等服务，
 * 都在真正动数据之前先调这里。</p>
 *
 * <p>它向下调用什么：什么都不调用。租户号、用户号、角色码都由调用方从可信上下文（登录态）取好后传进来。</p>
 *
 * <p>它不负责什么：不从 HTTP 请求里解析身份（浏览器传来的字段绝不能决定租户和角色）、
 * 不查询知识库实体、不做业务字段校验、不记审计日志。</p>
 */
@Service
