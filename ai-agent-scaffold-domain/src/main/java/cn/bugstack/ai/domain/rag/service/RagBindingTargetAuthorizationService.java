...
/**
 * 在把「某个知识库绑定到某个 Agent 或工作流」这条记录写进库之前，先确认这个绑定目标真实、属于本租户、且现在能跑。
 *
 * <p>解决什么问题：绑定关系决定了运行时会给哪个 Agent 或工作流注入哪些资料。
 * 如果允许随便填一个目标 ID 就建绑定，会出现三种坏情况：绑到一个不存在的目标（数据成了垃圾）、
 * 绑到别家租户的工作流（跨租户读资料的入口）、绑到一个还没发布或已禁用的目标（运行时必然报错）。
 * 这里在写库前一次性把这三种情况全部挡住。</p>
 *
 * <p>属于哪一层：领域层（domain）的授权与前置校验服务。只判断、只抛异常，不返回数据、不写库。</p>
 *
 * <p>谁会调用它：知识库管理服务在新增绑定时调用（RagKnowledgeBaseManagementService）。</p>
 *
 * <p>它向下调用什么：Agent 可用性服务（判断是否平台注册、是否被本租户禁用），
 * 以及工作流仓储（判断租户归属与发布状态）。</p>
 *
 * <p>它不负责什么：不校验操作人角色（那是知识库授权服务的事）、不校验知识库是否存在、不写绑定记录、
 * 不做运行时检索、也不在绑定建好之后持续监控目标是否仍然可用。</p>
 */
@Service
